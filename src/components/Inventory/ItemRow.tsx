...
export default function ItemRow({ item }: ItemRowProps) {
  ...
  return (
    <tr className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
      ...
      <td className="px-6 py-4">
        <div>
          <p className="font-medium text-slate-800">{item.quantity} {item.unit}</p>
          <p className="text-sm text-slate-500">{item.usageRatePerPersonPerDay}/person/day</p>
        </div>
      </td>
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              ...
              <p className="text-slate-600 mb-4">Are you sure you want to delete "{item.name}"? This action cannot be undone.</p>
