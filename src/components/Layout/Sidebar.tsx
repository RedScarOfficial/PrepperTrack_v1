...
export default function Sidebar({ activeTab, onTabChange }: SidebarProps) {
  return (
    <div className="w-64 bg-slate-800 dark:bg-slate-950 text-white min-h-screen">
      <div className="p-6 border-b border-slate-700 dark:border-slate-800">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Shield className="h-8 w-8 text-green-400" />
            <div>
              <h1 className="text-xl font-bold">PrepperTrack</h1>
              <p className="text-slate-400 text-sm">Survival Readiness</p>
            </div>
          </div>
          <NotificationCenter />
        </div>
